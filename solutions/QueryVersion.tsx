...
function UserList() {
    ...
    // TODO: 
    // Exercise 1: Implement data fetching with useQuery
...
const { data: users, isLoading, error, refetch, isFetching } = useQuery({
    ...
    // Configuration options: Those options will indicate when to auto-refetch,
    // when to consider that data is stale, and how long to cache it. This will
    // automatically handle the request to update the data, reducing boilerplate
    // and improving UX by keeping data fresh.
    refetchInterval: 30000,         // Poll (refetch) every 30 seconds
});
...
function UserCount() {
  const { data: users, isLoading } = useQuery({
    queryKey: ['users'], // Same key as UserList! 
    queryFn: () => {
      console.log('🟢 useQuery (UserCount): This should NOT run if UserList already fetched! ');
      return userAPI.getAll();
    },
  });
      ...
      <div className="bg-green-50 border-2 border-green-200 rounded-lg p-6">
        ...
        <div className="bg-white rounded p-4">
          ...
          <ul className="list-disc list-inside space-y-1 text-sm text-gray-700">
            <li>Open DevTools → Network tab: <strong>Only 1 request</strong> (automatic deduplication)</li>
            <li>Open Console: Only 1 fetch log</li>
            <li>Less boilerplate code</li>
            <li>Automatic caching:  Navigate away and back → instant load from cache</li>
            <li>Background refetching: Focus the window to trigger automatic refresh</li>
